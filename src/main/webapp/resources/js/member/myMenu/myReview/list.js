...
function makeList() {	
	var urlStr = _contextPath + '/member/myMenu/myReview/list.do';
	// 게시물 유형 코드(1=일정, 2=후기, 3=답변)
	postTypeCode = 2;
...
function makeListHtml(json) {
	...
	for (var i = 0; i < listCnt; i++) {
		...
		contentStr += "<a href='" + _contextPath + "/review/view.do?postSeq=" + review.postSeq + "' class='img img-2 d-flex justify-content-center align-items-center' ";		
		contentStr += "style='background-image: url(" + _contextPath + "/resources/" + review.savePath + "/" + review.saveName + ");'>";		
		...
		contentStr += "<h3><a href='" + _contextPath + "/review/view.do?postSeq=" + review.postSeq + "'>" + review.postTitle + "</a></h3>";		
		contentStr += "</div>";
	...
	// 일단 싹 지우고 리스트 추가
	$("#myList").children("div").remove();
	$("#myList").append(contentStr);
